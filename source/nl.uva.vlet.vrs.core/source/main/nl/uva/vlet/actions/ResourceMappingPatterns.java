 ...
 * The Implementation will definitaly change in the future. 
 * Use ActionMenuMapping object to specify (and add) action mappings. That interface
 * will remain stable.  
 * <p> 
 * Matches Resource Type (+optional Scheme) with selected ressourceType (+optional) resourceScheme.  
 * Resource(Type) is the resource which has the 'Focus', that is the resource which 
 * has been right clicked (or alt mouse button clicked) on. <br>
 * It May or May not be selected itself!
 * <p> 
 * The Selected Resources are the resources either in the clipboard (copy,cut,CTRL-C,CTRL-V) 
 * or explicit selected holding CTRL down in the gui or, in the case of an Drag and Drop, 
 * the resources being dragged onto the node matching 'resourceType' . 
 * The SelectionMode determines whether the Action is for one, more or no selected 
 * items in either the clipboard or active selected on the canvas.
 * <p>
 * Notes:  <br>
 * <li> 
 * <li> To create multiple actions for the <i>same</i> submenu, just specify more actions which 
 * have the same submenu name! (The gui will merge the entries). 
 * <li> A Drag and Drop action cannot be empty and thus cannot hase NONE Selected items.  
 */ 
class ResourceMappingPatterns
{
	...
	static abstract class ResourcePattern
	{
		...
		/**
		 * Matches Resource Triple: {Type,Scheme,MimeType}  
		 * It is up to the implemenation to ignore either type, scheme and/or mimetype. 
		 */ 
		abstract boolean matches(String typeVal,String schemeVal,String mimeType); 

		/** 
		 * Needed in super type so it can be used as first HashEntry !
		 * Returns list of allowed type or NULL if don't care. 
		 */ 
	...
	{	
		...
		/*public ResourceTypeSchemePattern(Pattern[] sourceTypes, Pattern[] sourceSchemes)
		{
		...
		/**
		 * Checks whether type and scheme are in the allowed values,
		 * ignores mimeType 
		 */ 
	...
	{	
		...
		boolean matches(String type, String scheme,String mimeType)
		{
			...
			if (StringUtil.isEmpty(mimeType))
				return false; // null not allowed must use RE ".*" for wildcards... 
	...
	/**
	 * SelectionMode. 
	 * Default: ignore nr of selected times
	 * Can be ONE, TWO, ONE_OR_MORE,etc. 
	 */
	 ...
	 * 
	 * (I) Check source type+scheme
	 * Pre check selections: 
	 * (IIa)  Selection Mode = DONT_CARE -> return true; 
	 * (IIb)  Selection Mode = NONE and no selections -> return true; 
	 * (IIc)  Selection Mode = other combinations  
	 * 
	 * (III) Post check
	 * 
	 */
	 public boolean matches(ActionContext selContext)
     {
		...
		//
		// (I) First check source, if it doesn't match, the selections don't need to be matched.
		//
		...
		// and filter out impossible combinations which can be detected before doing the match
		//
		...
		// current default is that all selected resource must match except if mode is DON'T CARE 
		if (matchall==false)
			return false;
	...
	private boolean exactNumberOfSelectionsPossible(int num)
	{
		...
		// So this method checks explicit modes (one explicit set Selection Flag). 
		// 
	...
	public static ResourceMappingPatterns createMimeTypeMappings(ActionMenuMapping owner, 
			Pattern[] sourceMimeTypes, Pattern[] selectionMimeTypes, int selectionMod)
	{
