...
//		}
	...
	private static void debug(String msg) {
		Global.debugPrintln(ResourceEditor.class, msg);
	}
	...
	public static void editProperties(final ProxyNode pnode,
			final boolean fullConfiguration) {
	...
	private Icon getDummyIcon() 
	{
		ImageIcon icon = new ImageIcon(this.getClass().getClassLoader()
				.getResource("web/icon.png"));
		return icon;
	}

	private AttrSchemeField createAttrSchemeField(String scheme, 
			String[] values, 
	...
	private AttrParameterField createAttributeField(String name,
			String dummyValue,
			IComboFocusActionListener attributeFocusActionListener)
	{
	...
	private AttrPortField createPortField(int dummyValue,
			IComboFocusActionListener attributeFocusActionListener)
	{
	...
	private AttrCheckBoxField createCheckBoxField(String name,
			boolean value,
			IComboFocusActionListener listener)
	{
	...
	private JLabel resourceSettingsLbl() {
		if (topPanelResourceLbl == null) 
		{
			...
			topPanelResourceLbl.setBorder(BorderFactory
	...
	private AttrParameterField getUriQueryField() 
	{
		...
		{
			uriQueryField = createAttributeField(
					VAttributeConstants.ATTR_URI_QUERY,
					"",
	...
	private AttrParameterField getUriFragmentField() 
	{
		...
		{
			uriFragmentField = createAttributeField(
					VAttributeConstants.ATTR_URI_FRAGMENT,
					"",
					this.controller.getAttributeListener());
	...
	private JCheckBox getUriAttrEnableCB() 
	{
		if (uriAttrEnableCB == null) 
		{
			...
			uriAttrEnableCB.setBorder(BorderFactory
	...
	private JPanel getTopPanel() {
		if (topPanel == null) {
			...
			topPanel.add(resourceSettingsLbl(), new CellConstraints(
					"2, 2, 8, 1, default, default"));
			{
				typeLabel = new JLabel();
				topPanel.add(typeLabel, new CellConstraints(
						"5, 5, 1, 1, default, default"));
				typeLabel.setText("type:");
			}
			{
				topPanel.add(getTypeField(), new CellConstraints(
						"7, 5, 2, 1, default, default"));
				typeField.setEditable(false);
			}
			{
				nameLabel = new JLabel();
				topPanel.add(nameLabel, new CellConstraints(
						"5, 6, 1, 1, default, default"));
				nameLabel.setText("name:");
			}
			{
				topPanel.add(getNameField(), new CellConstraints(
						"7, 6, 2, 1, default, default"));
				topPanel.add(getIconPanel(), new CellConstraints(
						"3, 4, 1, 4, default, default"));
			}
		}
		return topPanel;
	}
	...
	private JComponent getNameField() {
		if (nameField == null) {
			nameField = createAttributeField(VAttributeConstants.ATTR_NAME,
					"<name>",
					this.controller.getAttributeListener());
		}
		return nameField;
	...
	private JComponent getTypeField() {
		if (typeField == null) {
			typeField = createAttributeField(VAttributeConstants.ATTR_TYPE,
					"<TYPE>",
					this.controller.getAttributeListener());
		}
		return typeField;
	...
	private AttrParameterField getPathField() {
		if (pathField == null) {
			pathField = createAttributeField(VAttributeConstants.ATTR_PATH,
					"<PATH>",
					this.controller.getAttributeListener());
		}
		return pathField;
	...
	private AttrSchemeField getSchemeSB() {
		if (schemeSB == null) {
			schemeSB = createAttrSchemeField(
					VAttributeConstants.ATTR_SCHEME,
					new String[] { "<SCHEME1>", "<SCHEME2>" },
					this.controller.getAttributeListener());
		}
	...
	private AttrParameterField getHostnameField() {
		if (hostnameField == null)
			hostnameField = createAttributeField(
					VAttributeConstants.ATTR_HOSTNAME, "<HOSTNAME>",
					this.controller.getAttributeListener());
	...
	private AttributePanel getConfigAttrsPanel() {
		if (propertiesAttrPanel == null) {
			...
			propertiesAttrPanel.addAttributeListener(this.controller
	...
	private JPanel getTabIconAttrsPnl() {
		if (tabIconAttrsPnl == null) { 
			...
			FormLayout iconAttrsPanelLayout = new FormLayout(
					"5dlu, 36dlu, max(p;5dlu), 54dlu, 75dlu:grow, 8dlu", 
	...
	private AttrCheckBoxField getShowShortCutIconX()
	{
		...
		{
			showShortCutIconX = createCheckBoxField(
					VAttributeConstants.ATTR_SHOW_SHORTCUT_ICON,
					true,
					this.controller.getAttributeListener());
	...
	private AttrParameterField getIconUrlField()
	{
		if (iconUrlField == null)
			iconUrlField = createAttributeField(
	...
	private JPanel getPropertiesTopPanel() {
		if(propertiesTopPanel == null) {
			...
			FormLayout propertiesTopPanelLayout = new FormLayout(
					"max(p;5dlu), 63dlu, 98dlu:grow, max(p;5dlu)", 
	...
	private JPanel getServerConfigTopPnl() {
		if(serverConfigTopPnl == null) {
			...
			FormLayout serverConfigTopPnlLayout = new FormLayout(
					"max(p;5dlu), 42dlu, max(p;5dlu), 67dlu, 69dlu, 5dlu", 
	...
	private JPanel getTabLocationAttrsPnl() 
	{
		if(tabLocationAttrsPnl == null) 
		{
			...
			tabLocationAttrsPnl.setBorder(BorderFactory.createEtchedBorder(BevelBorder.LOWERED));
			{
				...
				locationAttrsPnl.setPreferredSize(new java.awt.Dimension(
						479, 177));
	...
	private JPanel getLocationTopPnl() {
		if(locationTopPnl == null) {
			...
			FormLayout locationTopPnlLayout = new FormLayout(
					"max(p;5dlu), 66dlu, 115dlu:grow, max(p;5dlu)", 
