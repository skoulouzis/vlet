...
// Important : keep import footprint minimal ! 
import nl.uva.vlet.data.StringList;
import nl.uva.vlet.data.StringUtil;
import nl.uva.vlet.vrl.VRL;

/** 
 * Class for Global system variables and runtime environment.
 * To configure the Global Environment use GlobalConfig. 
 * <p>
 * <b>Important</b>: To avoid Chicken&Egg initialization problems: <br>
 *  - Use GlobalConfig methods to configure pre-initialization properties 
 *    and Global methods after initialization (Global.init()) <br>
 * <br>  
 * As a rule "configurable" properties are in GlobalConfig. Static properties 
 * are in Global.  
 * <p>
 * Developers notes:<br>
 *  - Do not call Registry methods from this class(Chicken and Egg initialization)!  
 *    Registry calls Global methods !<br>
 *    
...
public class Global
{
    ...
    /**
     *  Private class initiliazer: use to initialize this class.
     *  Must not call Registry class as that class references THIS class. 
     */
     ...
     * want to instantiate a ClassLogger. 
     * Note that the overhead is slightly higher since extra checking might ocure. 
     * Use ClassLogger().getLogger(...) to instantiate a default Logger which is faster. 
     */
     ...
     * want to instantiate a ClassLogger. 
     * Note that the overhead is slightly higher since extra checking might occur. 
     * Use ClassLogger().getLogger(...) to instantiate a default Logger which is faster. 
     */
    ...
    /** Dump stacktrace to error output */ 
    ...
    // ==========================================================
    // Deprecated methods. Most methods will move to GlobalLogger
    // ==========================================================
    ...
    /**
     * @deprecated  Will move to GlobalLogger() !
     * Use GlobalLogger().getDefault() for more logging method 
     */ 
    ...
    /**
     * Get user property from $HOME/.vletrc/vletrc.prop
     * Will always reload ~/.vletrc/vletrc.prop file.
     * Return null value is file doesn't exist. 
     */ 
    ...
    /** 
     * Return property from INSTALLDIR/etc/vletrc.prop or 
     * vletrc.prop found on class path (backup properties).
     *  
     * @param name
     * @return
     */
     ...
     * Can be overriden if in the installation property the value
     * 'user.home' has been defined which may be a relative path 
     * to the installion for personalized installation settings.
     * <p>
    ...
    /**
     * Get VFS compatible User Home VRL. By avoiding location 
     * file paths and (re)use the VFS interface, file and 
     * configuration can be stored anywhere ! 
     ...
     * By avoiding location file paths and (re)use the VFS 
     * interface, file and configuration information 
     * can be stored anywhere ! 
     * Default is $HOME/.vletrc
     * @return
     */
     ...
     * Dynamically tries to figure out where it
     * is started from.
     * <p> 
     * If VLET_INSTALL is set, it will return this value as VRL.
     * else the startup directory will be used.   
     */
     ...
     * 
     * When the JVM is given a -Dprop=val argument, it is already
     * filtered, but when the argument is given after the 
     * start class, the argument is in the args list.
    ...
    /** 
     * Whether OS has an UX compatible FS.  
     * Currently only returns 'true' when operating system is Linux.
     * Mac OS has an UX like FS too.   
    ...
    /** @deprecated Configuration of (system) properties has been moved to GlobalConfig */ 
     ...
     * This means no incoming tcp/ip connections are allowed 
     * or possible. 
     * @return
     */
    public static boolean getPassiveMode()
    {
        GlobalConfig.passiveModeOnly=Global.getBoolProperty(GlobalConfig.PROP_PASSIVE_MODE,GlobalConfig.passiveModeOnly);
    ...
    /** 
     * Returns all root partition on this system. On Windows, this 
