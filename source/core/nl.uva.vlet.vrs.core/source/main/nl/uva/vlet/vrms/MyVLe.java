...
/**
 * MyVLe is the toplevel object for all resources, like My Computer
 * is on Windows. 
 * It's childs are configured locations and settings, etc. 
 *  
 * @author Piter T. de Boer
 *
 */
final public class MyVLe extends VCompositeNode implements VEditable, 
       VLogicalResource, VRenamable, VLogicalFolder
{
	// ========================================================================
	//  Class Stuff 
	// ========================================================================
	...
	/**
	 * Get "myvle:///" location. 
	 * This since myvle hasn't got a VRS location factory 
	 */ 
	 ...
	 * @uml.associationEnd   multiplicity="(0 -1)" elementType="nl.uva.vlet.vrs.LinkNode"
	 */
	...
	/** 
	 * The MyVLe object 
	 * @param vrsContext The VRSContext to use. 
	 * @throws VlException 
	 */
	private MyVLe(VRSContext context) 
	{
		super(context,new VRL(VRS.MYVLE_SCHEME,null,null));
		// rootGroup=new ResourceFolder(null,context,new VRL(VRS.MYVLE_SCHEME,null,null)); 
		...
		{
			// must continue!
			Global.errorPrintln(this,"Exception during initialization:"+e);
	...
	public synchronized String[] createResourceTypes()
	{
		...
		for (VRSFactory vrs:vrss)
		{
			...
			if ( (name2.startsWith("http"))
				 || (name2.startsWith("gat"))
				 || (name2.startsWith("info"))
			   )
			{
			   ; // skip
			}
	...
	private String attrNames[]=
	{
			...
			ATTR_TYPE,
	        "[User]",
	        GlobalConfig.PROP_USER_CONFIGURED_VOS, 
            "[FireWall Settings]",
			GlobalConfig.PROP_INCOMING_FIREWALL_PORT_RANGE,
            GlobalConfig.PROP_PASSIVE_MODE,
            "[BDII Service]",
            GlobalConfig.PROP_BDII_HOSTNAME,
            GlobalConfig.PROP_BDII_PORT,
            "[Installation]",
			GlobalConfig.PROP_VLET_VERSION,
			GlobalConfig.PROP_VLET_INSTALL,
			GlobalConfig.PROP_VLET_SYSCONFDIR,
	...
	public VAttribute getAttribute(String name)
	{
		...
		{
		    Global.errorPrintln(this,"Configuration Exception:"+e); 
		}
	...
	public boolean setAttribute(VAttribute attr) throws VlException
	{
		...
		{
			Debug("*** Error: setAttribute:unknown attribute:"+attr);
		}
	...
	private void readChilds(boolean autoinit) throws VlException 
	{
		...
		for (VFSNode child:childs)
		{
			try
			{
				...
				if (child instanceof VFile)
				{
					...
					if (file.getLocation().isVLink())
					{
						...
						// === LEGACY CODE === 
						// auto-update
						// when a link in an old format is detected/corrupted: it will be deleted 
						if (lnode.getAttribute(ATTR_SCHEME)==null)
						{
							Global.infoPrintln("MyVLe","Warning: deleting old or corrupt resource specification:"+lnode);
							lnode.delete(); 
						}
						else
						{
			...
			{
				Global.errorPrintln(this,"*** Error reading child resource:"+child);
				Global.errorPrintStacktrace(e); 
			}
	...
	private VDir getRootResourceDir(boolean autoinit) throws VlException
	{
		...
		if (vfs.existsDir(loc)==false)
		{
			...
			{
				Global.errorPrintln(this,"Couldn't initialize myvle!:"+loc); 
			}
		}
		else
		{
    ...
    /* Index nr of added resource, this is NOT the index into the vector
	 * since any manipulation of the vector will change the order and 
	 * invaliday the logical VLRs stored in the ResourceNodes
	 */ 
	...
	private void addWindowsStuff()
	{
		...
		for (String subpath:new String[]{"Favorites","My Documents" })
		{
			...
			{
				Global.warnPrintln(this, "Couldn't add Windows Resource:"+localPath);
		...
		synchronized(storageCreationMutex)
		{
			...
			while(nameExists==true)
			{
				...
				{ 
					...
					// already covered  
					//if (StringUtil.compare(nodeNameWithExt,newNameWithExt)==0) 
					//	nameExists=true; 
				}
	 ...
	 * Will fail isMove==true to avoid deleting of the Dropped Node. 
	 * 
	 */ 
	public VNode addNode(VNode sourceNode,String optNewName,boolean isMove) throws VlException 
	{
		...
		if ( (sourceNode instanceof ResourceFolder) 
			  || (ResourceFolder.isResourceFolderDescription(sourceNode)))
		{
			...
			// name in ReosurceLinkNode is stored as attribute, so rename can be done after save() 
			if (optNewName!=null) 
...
//			else
		...
		if (isMove) 
			throw new nl.uva.vlet.exception.NotImplementedException("Cannot move this kind of resource. Use Copy or Link instead:"+sourceNode); 
		...
		// name in LinkNode is stored as attribute, so rename can be done after save() 
		if (optNewName!=null) 
			lnode.renameTo(optNewName,false);
	...
	public VNode createNode(String type, String name, boolean force) throws VlException 
	{
		...
		{
			// ResourceFolder gnode=new ResourceFolder(this.rootGroup,this.vrsContext,null); 
...
//		}
	...
	protected VNode getSubNode(VRL location,boolean recurse) throws VlException
	{
		...
		if ((location==null) || (location.getScheme().compareTo(VRS.MYVLE_SCHEME)!=0))
			throw new nl.uva.vlet.exception.ResourceTypeNotSupportedException("Location is NOT a myvle:// location:"+location); 
