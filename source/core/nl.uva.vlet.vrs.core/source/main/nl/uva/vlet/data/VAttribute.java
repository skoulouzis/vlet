 ...
 * Currently the attributes are stored as (java) Strings so this class is not
...
public class VAttribute implements Cloneable, Serializable, Duplicatable<VAttribute>
{
	...
	// Optional implementation: int intVal long longVal float floatVal double doubleVal etc...
	...
	protected void copyFrom(VAttribute source)
	{
		...
		init(source.type, (source.name != null) ? new String(source.name) : "",
				(source.value != null) ? new String(source.value) : "");
    ...
    protected void initValue(Object value)
    {
        ...
        if ((value instanceof VRL) || (value instanceof URI)
	...
	/* public VAttribute(String value)
	 {
	 init(VAttributeType.STRING, null, value);
	 }*/
	 ...
	 * To Actual get the typed object used getValueObject() 
	 */
	 ...
	 * Other setValue methods convert the value to a String value 
	 * and use this one. 
	 * Developers node: All other setValue methods must call this.  
	 * 
	 * @param val
	 */
	...
	protected String _setValue(String val)
	{
		...
		if (type == VAttributeType.ENUM)
		{
		    // ? auto set, checks should already have been done before this code.  
		    //if (enumValues==null)
		    //{
	...
	public static VAttribute[] convertVectorToArray(
			Vector<VAttribute> attributes)
	{
		...
		switch (this.type)
		{
			...
			default:
				...
				if (attr.getValue() != null)
				{
					if (ignoreCase)
					{
						return value.compareToIgnoreCase(attr.getValue());
					}
					else
					{
						return value.compareTo(attr.getValue());
					}
				}
				else
					return 1; // this >> null  
	 ...
	 * Implementation might be changed in the future to 
	 * a generic VAttribute Help Text database  
	 */
	...
	/**
	 * Return value as actual Java Object with the specified type.
	 * This will return an object with a type compatible with VAttributeType class.     
	 */
