...
/**
 * Grid Proxy Util class. It is a wrapper class around a globus credential. It
 * provides convenient methods in handling grid proxies. The Grid Proxy class
 * needs a "GridCredentialProvider" which is the credential (proxy) factory
 * class. To allow for different proxy schemes, the GridCredentialProvider is a
 * abstract interface. It is necessary to register a GridCredentialProvider
 * first before the GridProxy class can create proxies.
 *
 * @see VGridCredential
...
public class GridProxy {
    ...
    /**
     * Find and load proxy given the VRS Context Always returns new GridProxy
     * object, but credentials may not be valid yet, if the proxy hasn't been
     * created.
     */
    ...
    /**
     * Update Provider with Configuration Settings. Takes properties from
     * VLET_INSTALL/etc/vletrc.prop and ~/.vletrc/vletrc.prop and updates the
     * Proxy Provider.
     */
    ...
    /**
     * Returns VO Name which will be used to create the credential Strips /Role
     * part from VOName if defined.
     */
    ...
    /**
     * Whether VOMS proxies are enabled. This doesn't mean the current proxy is
     * VO enabled
     */
    ...
    /**
     * Location of usercert.pem used to create this proxy. If credential hasn't
     * been created it will return the default usercert location.
     */
    ...
    /**
     * Location of current created proxy. If the proxy hasn't been created yet
     * this method returns getDefaultProxyFilename. If the credential has been
     * saved to a file, that location will be returned.
     */
    ...
    /**
     * Return's default proxyfilename. New proxies will be created at this
     * location.
     */
    ...
    /**
     * Load extra certificates from installation directory
     * VLET_INSTALL/etc/certificates and user configuration directory
     * ~/.vletrc/certificates
     */
    ...
    /**
     * (re)load the proxy and it's configuration Uses the VRSContext which whas
     * supplied to the constructor to fetch user settings.
     *
    ...
    /**
     * Use GlobusCredential as current credential. The GlobusCredential Provider
     * must be registered
     */
    ...
    /**
     * Set (default) parent directory which contains certificates
     * (usercert.pem+userkey.pem) to ne used when creating new proxies.
     */
    ...
    /**
     * Sets default path of proxy filename. New proxies will be saved to this
     * path.
     */
    ...
    /**
     * Sets the lifetime in hours of the proxy. Only applies for new created
     * proxies. If persistantSettings==true this will save the settings to
     * $HOME/.vletrc.prop s
     */
    ...
    /**
     * Save proxy settings to $HOME/.vletrc/vletrc.prop. NOTE: this method will
     * make the settings in this proxy as the user's defaults.
     *
     */
    ...
    /**
     * Reload proxy if it exists, If no proxy has been loaded it returns
     * withouth any exception. use isValid() after (re)load to check whether the
     * current proxy is valid.
     */
    ...
    /**
     * Return VO Name directly from credential. Might return null if no VOMS
     * attribute can be found or credential is invalid. Uset getDefaultVOName to
     * get the configured VO.
     */
...
//    }
    ...
    /**
     * Convert proxy into password protected keystore including the proxy
     * certificate chain.
     */
