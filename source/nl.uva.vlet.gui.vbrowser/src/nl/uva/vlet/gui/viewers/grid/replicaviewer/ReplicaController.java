...
public class ReplicaController implements ActionListener, ListSelectionListener, ListDataListener
{
    ...
    private void bgUpdateReplicas(
            final String[] newSEs,
            final String[] deleteSEs,
            final String[] unregSEs)
    {
        int todo=0; 
        if (newSEs!=null)
            todo+=newSEs.length; 
                ...
                try
                {
                    ...
                    for (VRL repVrl:vrls)
                    {
                        ...
                        try
                        {
                            ...
                            debug("replica attributes:"+attrs); 
            ...
            public void doTask()
            {
                try
                {
                    ...
                    for (VRL vrl:vrls)
                    {
                        // [I] Pre Emptive abort ! 
                        if (mustStop==true)
                           break; //  throw new VlInterruptedException("Interrupted");
