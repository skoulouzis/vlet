 ...
 * ---
 * $Id: testVFS.java,v 1.39 2011-11-25 13:45:11 ptdeboer Exp $  
 * $Date: 2011-11-25 13:45:11 $
 */ 
// source: 
...
import nl.uva.vlet.vfs.lfc.LFCDir;
import nl.uva.vlet.vfs.lfc.LFCFile;
import nl.uva.vlet.vfs.srm.SRMConstants;
...
public class testVFS extends VTestCase
{
    ...
    private VFile createFile(VFileSystem fs, VRL fullpath, boolean ignore) throws VlException 
    {
    ...
    public void testCreateAndDeleteFullpathFile() throws VlException
    {
        ...
        VRL filevrl = getRemoteTestDir().getVRL().append(nextFilename("testFileB"));
     ...
     * @throws VlException
     */
    public void testCreateDeleteFileWithSpaceAndListParentDir() throws VlException
    {
        if (this.getTestEncodedPaths() == false)
        {
            message("***Warning: Skipping test:testCreateDeleteFileWithSpaceAndListParentDir");
    ...
    private void testVFSNodeAttributes(VFSNode newFile) throws VlException
    {
        ...
        Assert.assertEquals("Both getType() and getAttribute(ATTR_TYPE) must return same value", newFile.getType(),
                newFile.getAttribute(ATTR_TYPE).getValue());
    ...
    private VDir createDir(VFileSystem fs, VRL dirpath, boolean ignore) throws VlException 
    {
    ...
    private void testRename(VDir parentDir, String orgFileName, String newFileName) throws VlException
    {
        ...
        try
        {
            ...
            if (Global.isLinux() == true)
            {
                if (root.isWritable()==false)
                {
                    ...
                    fail("Should raise Exception:" + ResourceWriteAccessDeniedException.class);
                }
                //; continue
            }
    ...
    public void testStreamWriteTruncates() throws VlException
    {
        ...
        verbose(1, "streadWriting to:" + remoteFile);
        remoteFile.streamWrite(buffer, 0, buffer.length);
        assertEquals("Initial remote file size NOT correct", remoteFile.getLength(), len);
        ...
        assertEquals("New file size should be truncated after Stream Write!", len, remoteFile.getLength());
    ...
    public void testMultiStreamWritesCheckLengths() throws VlException
    {
        ...
        for (int i = 0; i < numTries; i++)
        {
            ...
            assertEquals("LFC File size NOT correct", remoteFile.getLength(), testSize);
            ...
            if (remoteFile instanceof VReplicatable)
            {
                ...
                assertEquals("LFC Replica File size NOT correct", repLen, testSize);
            }
    ...
    private void _testRandomReadable(VRandomReadable rfile, int offset, int end, byte orgBuffer[]) throws VlException
    {
    ...
    public void testVRandomReader() throws VlException
    {
        ...
        assertEquals(contents.substring(0, signatureBytes.length), new String(signatureBytes));
      ...
      //-----------Test skip 
        ...
        assertEquals(contents.substring(lenToSkip,lenToSkip+1), new String(signatureBytes));
    }
    ...
    public void testReadWriteRandomBytes() throws VlException
    {
        ...
        // use syncReadBytes!
        int numRead=StreamUtil.syncReadBytes(randomWriter,0, buffer2, 0, nrBytes);
    ...
    private void testReadWriteBytes(VFile newFile, long offset, byte[] buffer1) throws VlException
    {
        VRandomAccessable randomWriter = null;
     ...
     */
    public void testZRegressionLFCZeroFileSize() throws VlException
    {
     ...
     */
    public void testZForceDeleteCorruptedLFCFile() throws VlException
    {
        VDir remoteDir = getRemoteTestDir();
     ...
     * 
     */
    public void testZRegressionSRMStorageType() throws VlException
    {
        VDir remoteDir = getRemoteTestDir();
    ...
    public void testZExceptionCreateFileDoNotIgnoreExisting() throws VlException
    {
    ...
    public void testZExceptionCreateDirNotIgnoreExsting() throws VlException
    {
        ...
        try
        {
            ...
            fail("Should raise and Exception. Preferably:" + ResourceAlreadyExistsException.class);
        }
    ...
    public void testZCreateDirectoryWhileFileWithSameNameExists() throws VlException
    {
        ...
        if (getRemoteTestDir().existsDir(fdname))
        {
            ...
            dir.delete(false);
            assertFalse("Could not remote previous test directory. Please remove it manually:" + dir, dir.exists());
        ...
        // MUST return false!
        assertFalse("existsDir() must return FALSE when file with same name already exists!", getRemoteTestDir()
    ...
    public void testZCreateFileWhileDirectoryWithSameNameExists() throws VlException
    {
        VDir newDir = getRemoteTestDir().createDir("testfiledir3");

        // MUST return false!
        assertFalse("existsFile() must return FALSE when directory with same name already exists!", getRemoteTestDir()
    ...
    public void testZExceptionsExistingDir() throws VlException
    {
        ...
        try
        {
            ...
            newDir.delete();
            fail("createDir(): Should raise Exception:" + ResourceAlreadyExistsException.class);
        }
        catch (ResourceAlreadyExistsException e)
        {
            Global.debugPrintln(this, "Caugh expected Exception:" + e);
        ...
        try
        {
            ...
            newDir.delete();
            fail("createDir(): Should raise Exception:" + ResourceAlreadyExistsException.class + " or "
