...
/** 
 * Server/Service side implementation of the ProxyVRS.  
 * All methods are entered through the 'invoke' method. 
 * 
...
public class ProxyVRSImpl
{
    ...
    public Object invoke(Class<?> resultClass, String resourceVRL, String methodName, Object[] args) throws VlException
    {
        ...
        if (args!=null)
            for (Object arg:args)
                if ((arg instanceof Serializable)==false)
                    throw new VlInvokeException("Object class is NOT Serializable:"+args.getClass().getCanonicalName()); 
        ...
        if ((result instanceof Serializable)==false)
            throw new VlInvokeException("After method:"+methodName
                    +". Result class is NOT Serializable:"+result.getClass().getCanonicalName());
    ...
    private VRL _create(String resourceVRL, 
            String type,
            String name,
            Boolean ignoreExisting) throws VlException
    {
    ...
    private void checkArgs1(Object[] args,Class<?> class1) throws VlInvokeException
    {
        ...
        if (class1.isInstance(args[0])==false)
            throw new VlInvokeException("Class type mismatch for 1st argument. Got:"+args[0].getClass().getCanonicalName()+", expected:"+class1.getCanonicalName());
    }
    
    private void checkArgs2(Object[] args,
            Class<?> class1,
            Class<?> class2) throws VlInvokeException
    {
        if (args==null)
            throw new VlInvokeException("Received EMPTY (Null) argument list. Expected 1 arguments");
        if (args.length!=2)
        ...
        if (class2.isInstance(args[1])==false)
            throw new VlInvokeException("Class type mismatch for 2d argument. Got:"+args[0].getClass().getCanonicalName()+", expected:"+class1.getCanonicalName());
    }
    private void checkArgs3(Object[] args,
            Class<?> class1,
            Class<?> class2,
            Class<?> class3) throws VlInvokeException
    {
        if (args==null)
            throw new VlInvokeException("Received EMPTY (Null) argument list. Expected 1 arguments");
        if (args.length!=3)
        ...
        if (class3.isInstance(args[2])==false)
            throw new VlInvokeException("Class type mismatch for 3rd argument. Got:"+args[0].getClass().getCanonicalName()+", expected:"+class1.getCanonicalName());
    }
    ...
    private void checkArray(Object[] args,Class<?> class1) throws VlInvokeException
    {
        ...
        for (int i=0;i<args.length;i++)
            if (class1.isInstance(args[i])==false)
                throw new VlInvokeException("Class type mismatch for argument #"+i+". Got:"+args[i].getClass().getCanonicalName()+", expected:"+class1.getCanonicalName());
    
    }
    
    
    private VRL _rename(String resourceVRL, 
            String nameOrPath,
