...
/** 
 * Registry for the Server (Resource) Info objects.
 * This information can be used to describe all kinds of 'Resources'.   
 * <p>
 * If a server provides more services then per service a ServerInfo object 
 * has to be registered.  
 * For example for ssh and gftp, one object has to be stored
 * for the ssh 'server' and one for the 'gftp' server. 
 * The same yields for multiple 'user' accounts. Each account (username) has 
 * to have a different 'ServerInfo' object. 
 * 
 * Currently the following Server field are use to uniquely identify a ServerInfo object:
 * <p>
 ...
 * <li> Userinfo :  This is the username or the username+domainname information
 *                  in the case of SRB Resources.
...
{
	private static final String XML_SERVER_CONFIG_HEADER = 
	     " VLET ServerRegistry Configuration\n"
    ...
    /**
	 * Moved hashtable with ServerInfo objects to seperate registry to 
	 * enable VRSContext dependend server information. 
	 */
    ...
    public ServerInfo getServerInfo(String infoID)
    {
        ...
        // Global.messagePrintln(ServerInfo.class,"searching for:"+accountID);
     ...
     * Is used to find *any* server of the specified protocol.   
     * @param scheme
    ...
    public ServerInfo getServerInfoForScheme(String scheme)
    {
        ...
        {
            debug("*** Warning:couldn't find server info for scheme:"+scheme);
        	return null;
        }
    ...
    /**
     * Return ServerInfo objects for the specified Scheme. 
     * Is used to find *any* server of the specified protocol.   
     * @param scheme
     * @return ServerInfo object
     */
    ...
    /**
     * Get All Server descriptions for this host. 
     * This allows grouping of Services per host. 
     */
    ...
    /**
     * Main method to search for a Server Info object.
     * When a search field is null it means "don't care". 
     * Use port number less then 0 for a don't care.  
     * Use port number EQUAL to 0 for default port ! 
     * Returns multiple matching ServerInfo descriptions or NULL when 
     * it can't find any matching server descriptions.
     * 
     * @param scheme    if NULL then don't care (match any)   
     * @param hostname  if NULL then don't care (match any) 
     * @param port      if port==-1 => don't care, if port==0 => default and if port>0 match explicit port number
    ...
    {
        debug(">>> find {scheme,host,port,user}={"+scheme+","+host+","+port+","+optUserInfo+"}"); 
//        if (scheme.compareTo("srb")==0)
//        {
//            Global.debugPrintln(this,"SRB"); // breakpoint
//        }
        ...
        if (result.size()<=0)
        {
            debug("<<< Returning NULL ServerInfos"); 
            return null;
        }
        
        ServerInfo arr[]=new ServerInfo[result.size()];
        arr=result.toArray(arr);
        debug("<<< Returning "+arr.length+" ServerInfos"); 
	...
	public void removeAll()
    {
	    synchronized(this.serverInfos)
        {
	        ...
	        debug(">>> ServerInfos.clear() ! <<<");
            serverInfos.clear();
        }
    }
    
	/**
	 * Stored new ServerInfo and writes to file (if persistant)
	 * Returnes updated serverInfo
	 */
	public ServerInfo store(ServerInfo info)
	{
	    debug("store(): attrs="+info.getAttributeSet());
	    
	    synchronized(this.serverInfos)
        {
	        // ===
	        // Before adding new Info: check whether persistant database is loaded ! 
	        // ===
	        checkIsLoaded();
	        // remove before put!
	        remove(info);
	        ...
	        //enters mutex again
	        save(); 
	        return info.duplicate(); // do NOT return reference into ServerRegistry 
        }
    ...
    protected void checkUpdateID(ServerInfo info)
    {
        synchronized(this.serverInfos)
        {
            ...
            String fixedID=userInf+info.getScheme().toUpperCase()+"-"+StringUtil.noNull(info.getHostname()).toUpperCase()+":"+info.getPort();
            info.setID(fixedID); 
        }
    }
...
//            }
	...
	private void put(ServerInfo info)
    {
	    synchronized(serverInfos)
	    {
	        ...
	        debug("+++) Storing info:"+info); 
	        
	        ServerInfo prev = this.serverInfos.get(info.getID());
	        
	        if (prev==info)
	            Global.infoPrintln(this,">>> updating ServerInfo:"+info);
            else if (prev!=null)
                Global.infoPrintln(this,">>> WARNING: Overwriting previous object with:"+info); 
            else 
                Global.infoPrintln(this,">>> storing new ServerInfo:"+info);
    ...
    /** 
	 * Checks whether the persistent Server Info Registry is loaded 
	 * and load it if necessary. 
	 */
	private void checkIsLoaded()
    {
	    debug("checkIsLoaded():"+isLoaded); 
	    ...
	    synchronized(this.serverInfos)
	    {
            ...
            {
                Global.warnPrintln(this,"Warning: Couldn't load ServerInfo Registry:"+e); 
            }
	    }
	...
	/**
	 * Find ServerInfos for specified location, 
	 * this can be 0 or more matches ! 
	 */ 
	public ServerInfo[] getServerInfosFor(VRL loc)
	{
		...
		if (infos!=null)
			if	(infos.length>1)
				debug("*** Warning: matched more then 1 ServerInfo for location:"+loc);
			else
				debug(">>> returning 1 info:"+infos[0]);
	...
	public ServerInfo remove(ServerInfo info)
	{
		...
		Global.infoPrintln(this,"--- Removing ServerInfo:"+info); 
		 
		ServerInfo prev=null; 

		synchronized(this.serverInfos)
		{
			String key=info.getID(); // return NULL if key hasn't been set/info hasn't been stored !
			if (key!=null)
	...
	/**
	 * Saves the ServerInfo Registry if the current configuration 
	 * allows a persistant Registry !
	 *  
	 * By default this is false, but when MyVle is initialized by 
	 * the VBrowser the persistant ServerInfo Registry will be enabled. 
	 * 
	 */
	public void save()
    {
	    ...
	    //xmlifier.setPersistanteNodeElementName("vlet:ServerInfo2"); // No Nodes!
	    try
        {
        ...
        {
        	Global.warnPrintf(this,"Couldn't save server registry to:%s\n",loc);
        	// but continue!
        }
    }
	 ...
	 * 
	 * Currently when MyVLe object is initialized the ServerInfoRegistry
	 * will be loaded. 
	 */
	protected void load() throws VlException
	{
	    ...
	    synchronized(this.serverInfos)
        {
            ...
            {
                Global.debugPrintln(this,"Adding ServerInfo Set:"+set); 
            }
            ...
            for (VAttributeSet set:sets)
            {
                ...
                debug("Adding Server Config:"+info);
                // actual store method: 
                put(info); 
            }
