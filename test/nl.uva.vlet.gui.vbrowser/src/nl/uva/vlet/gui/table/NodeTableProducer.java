...
/**
 * Produces table data. 
 * Todo:Use tablemodel and update model instead of creation complete table
 * each time. 
 *  
 */
    ...
    TableDataProducer, IProxyNodeEventListener
{
    ...
    protected synchronized void createTable1(ProxyNode rootNode,
            ProxyNode nodes[])
    {
    	//
	  ...
	  protected void asyncUpdateTableNodeAttributes()
	    {
	        ...
	        this.fetchTask = new ActionTask(getMasterBrowser(),
	                "TablePanel.UpdateAttributes:" + this.rootNode.getVRL())
	        {
	            ...
	            protected void doTask()
	            {
	                ...
	                for (int i = 0; (i < nodes.length) && (thisThread != null); i++)
	                {
	                    ...
	                    try
	                    {
	                        ...
	                        // use synchronized method, we are already in seperate subthread:
	                    ...
	                    catch (Exception e)
	                    {
	                    	...
	                    	{
	                    		Global.errorPrintln(this,"To many exceptions:"+e); 
	                    	}
	                    }
	                }
	...
	private void setNodeAttribute(ProxyNode pnode, VAttribute attr)
    {
        ...
        if ((col < 0) || (row < 0))
        {
            ...
            Global.errorPrintln(this, "setNodeAttribute: Invalid row/column:" + col + "," + row);
            return;
        }
    ...
    public void notifyProxyEvent(ResourceEvent e)
    {
        ...
        {
            // TODO: 
            Global.infoPrintln(this,"Warning: Node not in cache:"+e.getSource());
        }
    ...
    protected void addChilds(VRL[] allChilds)
    {
        ...
        //try
        {
            ...
            for (int i=0;i<newChilds.size();i++) 
            {
                ...
                if (newNodes[i]==null)
                {
                    infoPrintf("Warning: addChilds(): New location not in proxy cache cache:%s\n",childLoc);
	...
	@Override
	public void updateChildNodesFor(ProxyNode parent, ProxyNode[] childs,
			boolean cumulative)
	{
		...
		{
			Global.errorPrintln(this,"Received update NOT for me:"+parent);
			return;
		}
